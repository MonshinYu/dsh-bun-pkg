...
if (!process.env.DSH_BUN_COMPAT_CHILD && process.argv.some((value) => value.endsWith("@deepseek-ai/dsh/lib/bin.js"))) {
  const compatLib = resolve2(import.meta.dir);
  const root = findWorkspaceRoot(process.env.DSH_BUN_COMPAT_ROOT ?? compatLib);
  const cache = prepareShadow(root, compatLib);
  const entry = join2(cache, "node_modules/@deepseek-ai/dsh/lib/bin.js");
  const hook = join2(compatLib, "runtime-hook.js");
  let exitCode = 1;
  try {
    ...
    rmSync2(cache, { recursive: true, force: true });
  }
